...
ALTER TABLE narratives ADD COLUMN user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;
...
DROP POLICY IF EXISTS "Allow public read" ON narratives;
DROP POLICY IF EXISTS "Allow public insert" ON narratives;
DROP POLICY IF EXISTS "Allow public update" ON narratives;
DROP POLICY IF EXISTS "Allow public delete" ON narratives;
